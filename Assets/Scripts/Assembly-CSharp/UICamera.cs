...
public class UICamera : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		else if (BuildDetails.Instance._UseLeapIfAvailable)
		{
			useMouse = true;
			useTouch = true;
		}
		mMouse[0].pos.x = ClayJamInput.CursorScreenPosition.x;
		mMouse[0].pos.y = ClayJamInput.CursorScreenPosition.y;
	...
	private void FixedUpdate()
	{
		...
		{
			hoveredObject = ((!Raycast(ClayJamInput.CursorScreenPosition, ref lastHit)) ? fallThrough : lastHit.collider.gameObject);
	...
	public void ProcessMouse()
	{
		...
		if (!flag)
		{
			for (int i = 0; i < 3; i++)
			{
				if (ClayJamInput.CursorButton(i) || ClayJamInput.CursorButtonUp(i))
				{
					flag = true;
					break;
				}
		...
		mMouse[0].pos = ClayJamInput.CursorScreenPosition;
		...
		for (int k = 0; k < 3; k++)
		{
			if (ClayJamInput.CursorButton(k))
			{
				flag3 = true;
				break;
			}
		...
		for (int l = 0; l < 3; l++)
		{
			bool flag4 = ClayJamInput.CursorButtonDown(l);
			bool unpressed = ClayJamInput.CursorButtonUp(l);
